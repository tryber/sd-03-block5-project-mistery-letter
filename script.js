window.onload = () => {
  ...
  document.getElementById("criar-carta").addEventListener("click", () => {
    let randomNumber = (max) => Math.trunc((max) * Math.random());
    let randomClass = () => `${grupoEstilo[randomNumber(grupoEstilo.length)]} ${grupoTamanho[randomNumber(grupoTamanho.length)]} ${grupoRotacao[randomNumber(grupoRotacao.length)]} ${grupoInclinacao[randomNumber(grupoInclinacao.length)]}`
